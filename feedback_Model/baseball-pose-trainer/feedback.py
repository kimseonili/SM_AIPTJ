...
class BodyValue:
    ...
    def startCompare() :
        ...
        if distance_list[0] > 49.0 and distance_list[2] >  500.0:
            feedback.append("하체가 불안정합니다. 타격시 다리사이간격 좀 더 벌려주세요")

        if distance_list[1] > 1.245:
            feedback.append("타격시 허리의 회전이 부족합니다. 앞발에 무게중심을 실고 허리를 회전시켜야합니다")

        if distance_list[3] > 608.0 :
            feedback.append("스윙 전 배트를 들고있는 팔이 몸에서 너무 뒤로 빠져있습니다 몸쪽으로 더 당겨주세요")

        if distance_list[5] > 360.0 and distance_list[6] > 323.0:
            feedback.append("스윙시 배트를 들고있는 팔은 최대한 몸에 붙혀서 끝까지뻗어서 스윙해주세요")
